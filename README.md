...
| パッケージ | 内容 |
| --- | --- |
| [`@insession/space`](./packages/space) | **space の親パッケージ。** 依存ゼロで、ヘッドレスなスペースを extension の集合として組み立てる。extension 契約（`defineSpaceExtension`）・集約 registry・room ライフサイクル（members / presence / 入退室）・インスタンス（`createSpace`）を持つ。WS サーバーとストレージは利用者が自前で持ち、こちらは I/O を一切行わず effect 記述子を返すだけ。**グローバルなアプリ一覧を持たず（extensions 配列が唯一の真実）**、状態は `name` で名前空間化される |
