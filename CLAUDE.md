...
| `packages/extension-pomodoro` | `@insession/extension-pomodoro` | 依存ゼロのポモドーロタイマー状態機械（server-authoritative。`reduce` は `{ state, effects }` を返す純関数で、一行宣言の永続化は effect 記述子としてホストに委ねる。`restore`/`persistState` で永続化境界を扱う） | **なし** |
| `packages/extension-whiteboard` | `@insession/extension-whiteboard` | 依存ゼロのホワイトボード状態機械（server-authoritative。自由描画の strokes/shapes と「お絵かき伝言ゲーム」relay を同居させた `reduce` は `{ state, effects }` を返す純関数で、完成アルバムの永続化は effect 記述子としてホストに委ねる） | **なし** |
...
**⚠ 4つの状態機械はすべて `reduce`/`onTimer` が `{ state, effects }` を返す。** 最初は `extension-pomodoro` / `extension-whiteboard` だけが素の state を返していたが、どちらにも「セッションを跨いで残すべきもの」が1つずつあった（一行宣言と完成アルバム）。それを消費側が**前後の state を差分して推測**していたので、遷移そのものが effect として言う形へ揃えた（`0.3.0`）。**新しい状態機械を足すときも、素の state を返す形にしないこと** — 「今は副作用が無い」パッケージでも、後から1つ生えた瞬間に消費側へ差分ロジックが生まれる。副作用が無いなら `effects: []` を返せばよい。
