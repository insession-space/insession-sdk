...
"@insession/space-state-react": minor
---

InSession 本体（`insession-space/insession-app`）の `packages/space-state` / `packages/space-state-react` から、独立したリポジトリ `insession-space/insession-sdk` へ移設した初版。

ロジック（`index.ts` / `state.ts` / `reduce.ts` / `actions.ts` / `store.ts` / `plugin.ts` / `effects.ts` / `reactions.ts` / `chat-lines.ts` / `presence.ts` / `types.ts` / `reduce.test.ts`、および `space-state-react/index.ts`）は1文字も変えていない。
